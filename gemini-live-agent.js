      ...
      setup: {
        ...
        generationConfig: {
          ...
          speechConfig: {
            voiceConfig: {
              prebuiltVoiceConfig: {
                voiceName: 'Kore'  // Natural-sounding voice
              }
            }
          }
        },
...
- Match the speaker's language (English or Chinese/Mandarin)
- Be natural and conversational
- If you need to look something up or perform a task, use the ask_assistant tool
- Introduce yourself briefly when you first join

You can hear the meeting participants. Respond naturally to what they say.`
          }]
        },
        tools: [{
          functionDeclarations: [{
            name: 'ask_assistant',
            description: 'Ask the OpenClaw AI assistant for help with tasks, lookups, or complex questions that need the full AI brain. Use this when you need to search, calculate, check calendars, or do anything beyond simple conversation.',
            parameters: {
              type: 'OBJECT',
              properties: {
                query: {
                  type: 'STRING',
                  description: 'The question or task to send to the AI assistant'
                }
              },
              required: ['query']
            }
    ...
    if (msg.serverContent?.modelTurn?.parts) {
      for (const part of msg.serverContent.modelTurn.parts) {
        if (part.inlineData?.mimeType?.includes('audio') && part.inlineData.data) {
          ...
          if (this.onAudioOut) this.onAudioOut(pcmBuf);
        }
        if (part.text) {
...
async function main() {
  ...
  wss.on('connection', (ws) => {
    ...
    // Audio from Gemini → Telnyx (convert PCM 24kHz → µ-law 8kHz)
    gemini.onAudioOut = (pcmBuf) => {
      try {
        const ulaw = pcm24kToUlaw8k(pcmBuf);
        // Send as Telnyx media format
  ...
  app.post('/webhook', (req, res) => {
    ...
    if (eventType) console.log(`📡 ${eventType}`);
  });
  ...
  // Enable media streaming if not auto-started
  try {
    await telnyxApi('POST', `/calls/${callControlId}/actions/streaming_start`, {
      stream_url: `wss://${tunnel.url.replace('https://', '')}/media`,
      stream_track: 'both_tracks',
    });
    console.log('🎵 Media streaming enabled');
  } catch (err) {
    console.log('⚠️ Streaming start:', err.message.slice(0, 100));
  }
